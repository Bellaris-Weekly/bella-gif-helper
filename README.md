...
在哔哩哔哩直播或视频页面中截取画面、编辑片段并导出 GIF 的用户脚本。当前版本为 `1.5.13`。
...
- `高`：256 色调色板、误差扩散抖动和无损结构优化。
- `中`：256 色调色板和轻度有损压缩，默认选择。
- `低`：256 色调色板和更高压缩强度。
...
标题旁显示当前版本号，页面启动时静默检查更新。点击版本号可重新检查；发现新版后显示红点与细红框，再次点击打开脚本安装页，确认更新后刷新原页面生效。网络失败不会弹窗，可点击重试。

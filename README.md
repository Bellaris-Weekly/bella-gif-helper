...
在哔哩哔哩直播或视频页面中截取画面、编辑片段并导出 GIF 的用户脚本。当前版本为 `1.3.1`。
...
导出时会按设备核心数启用 2 至 4 个编码 Worker，并限制每个 Worker 最多积压 2 帧。画面提取、裁剪缩放、文字与圆角合成、调色板映射及帧编码均在受控队列中完成；后台预览缓存固定在 16 MB 预算内，进入编辑器后逐步构建，不再通过隐藏 GIF 编码任务估算体积。

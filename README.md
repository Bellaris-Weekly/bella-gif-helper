...
在哔哩哔哩直播或视频页面中截取画面、编辑片段并导出 GIF 的用户脚本。当前版本为 `1.3.2`。
...
- 裁剪时间范围与画面比例
- 裁剪交互期间保持视频播放，只暂停后台取帧，松手后立即平滑放大并在空闲时恢复后台任务
- 直播 GIF 文件名使用最终首帧的直播时间，并标记主播名与房间号
- 添加文字图层并调整字号、颜色、描边和位置
- 实时预览导出分辨率、帧率和播放速度
- 拖动时间轴时先显示清晰缓存帧，再用原片高清帧精确覆盖
- 通过“乃 / 贝 / 然”三档控制 GIF 画质
- 导出透明圆角 GIF
- 使用全片调色板、并行编码和后压缩缩短导出时间并减少 GIF 体积
...
导出时会按设备核心数启用 2 至 4 个编码 Worker，并限制每个 Worker 最多积压 2 帧。画面提取、裁剪缩放、文字与圆角合成、调色板映射及帧编码均在受控队列中完成；后台预览缓存固定在 32 MB 预算内，进入编辑器后逐步构建，不再通过隐藏 GIF 编码任务估算体积。

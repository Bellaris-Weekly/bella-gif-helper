...
(() => {
  ...
  var init_define_SCRIPT_METADATA = __esm({
    "<define:__SCRIPT_METADATA__>"() {
      define_SCRIPT_METADATA_default = { version: "1.5.13", updateURL: "https://share.bellaris.fans/bella-gif-helper.user.js", downloadURL: "https://share.bellaris.fans/bella-gif-helper.user.js" };
    }
  });
  ...
  var require_panel = __commonJS({
    "src/ui/panel.css"(exports, module) {
      module.exports = `
  :host {
    all: initial;
    --color-bg: #151619;
...
`;
  ...
  var require_main = __commonJS({
    "src/main.js"() {
      ...
      var LIVE_FRAME_CHANNEL = "bella-gif-helper-live-frame-v1";
      var GIF_TRANSPARENT_INDEX = 255;
      var TIMELINE_THUMBNAIL_COUNT = 8;
      var TIMELINE_THUMBNAIL_WIDTH = 160;
      var TIMELINE_THUMBNAIL_HEIGHT = 90;
      var PALETTE_SAMPLE_WINDOW_FRAMES = 6;
      var PALETTE_SAMPLE_WINDOW_COUNT = 3;
      var PALETTE_SAMPLE_MAX_EDGE = 192;
      var PANEL_MIN_WIDTH = 160;
      var PANEL_MAX_WIDTH = 720;
      var PANEL_MIN_HEIGHT = 360;
      var EXPORT_PHASE_RANGES = Object.freeze({
        palette: Object.freeze([0, 12]),
        extracting: Object.freeze([12, 68]),
        encoding: Object.freeze([68, 88]),
        compressing: Object.freeze([88, 99])
      });
      var GIF_QUALITY_PRESETS = Object.freeze({
      ...
      function formatShortcut(shortcut) {
        ...
        const key = normalized.code.replace(/^Key/, "").replace(/^Digit/, "").replace(/^Numpad/, "Num ").replace(/^Arrow/, "Arrow ").replace(/^([A-Z])$/, "$1");
      ...
      function matchesShortcut(event, shortcut) {
        ...
        return event.code === normalized.code && Boolean(event.ctrlKey) === normalized.ctrlKey && Boolean(event.altKey) === normalized.altKey && Boolean(event.shiftKey) === normalized.shiftKey && Boolean(event.metaKey) === normalized.metaKey;
      }
      function isEditableShortcutTarget(target) {
        if (!target) return false;
        const tagName = String(target.tagName || "").toLowerCase();
        return tagName === "input" || tagName === "textarea" || tagName === "select" || Boolean(target.isContentEditable) || String(target.contentEditable || "").toLowerCase() === "true";
      }
      ...
      function createExportTiming(start, end, fps, speed = 1) {
        ...
        const sourceFrameStep = frameCount < requestedFrames ? duration / frameCount : 1 / Math.max(1, Number(fps) || 1);
        ...
        const frameTimes = Object.freeze(Array.from(
          { length: frameCount },
          (_, index) => Math.min(Number(end) - 1e-3, Number(start) + index * sourceFrameStep)
        ));
        const frameDelays = Object.freeze(boundaries.slice(1).map((boundary, index) => (boundary - boundaries[index]) * 10));
      ...
      function calculateTimelineViewport(start, end, duration) {
        const clipDuration = Math.max(1e-3, Number(duration) || 1e-3);
        const viewStart = clamp(Number(start) || 0, 0, clipDuration);
        const viewEnd = clamp(Number(end) || clipDuration, viewStart, clipDuration);
        if (viewEnd - viewStart < 1e-3) return Object.freeze({ start: 0, end: clipDuration });
      ...
      function calculateTimelinePosition(time, viewStart, viewEnd) {
        const duration = Math.max(1e-3, Number(viewEnd) - Number(viewStart));
      ...
      function calculateTimelineTime(position, width, viewStart, viewEnd) {
        ...
        return Number(viewStart) + ratio * Math.max(1e-3, Number(viewEnd) - Number(viewStart));
      }
      function calculateTimelineDragTime(anchorTime, deltaPx, width, viewStart, viewEnd, duration) {
        const clipDuration = Math.max(1e-3, Number(duration) || 1e-3);
        const viewSpan = Math.max(1e-3, Number(viewEnd) - Number(viewStart));
      ...
      function createPaletteSampleWindows(frameTimes, windowSize = PALETTE_SAMPLE_WINDOW_FRAMES) {
        ...
        const windows = [0, middle, times.length - size].map((start) => Object.freeze(times.slice(start, start + size)));
      ...
      function sanitizeFileNamePart(value, fallback = "") {
        const cleaned = String(value || "").replace(/[\u0000-\u001f<>:"/\\|?*]+/g, "_").replace(/\s+/g, "_").replace(/_+/g, "_").replace(/^[_\.]+|[_\.]+$/g, "");
        return cleaned || fallback;
      }
      function calculateLiveFirstFrameTime(liveWallClockStartMs, trimStart) {
        const start = Number(liveWallClockStartMs);
        if (!Number.isFinite(start)) return null;
        return start + Math.max(0, Number(trimStart) || 0) * 1e3;
      }
      ...
      function isLiveFrameMessage(event, expectedOrigin, expectedSource = null) {
        return Boolean(
          event && event.origin === expectedOrigin && (!expectedSource || event.source === expectedSource) && event.data?.channel === LIVE_FRAME_CHANNEL && event.data?.version === 1
        );
      }
      ...
      function toVideoOnlyMimeType(mimeType) {
        ...
        const codecs = (codecsMatch[1] || codecsMatch[2] || codecsMatch[3] || "").split(",").map((codec) => codec.trim()).filter(Boolean);
        const videoCodecs = codecs.filter((codec) => /^(?:avc1|avc3|hev1|hvc1|av01|vp0[89]|vp9|dvhe|dvh1)(?:\.|$)/i.test(codec));
      ...
      function parseLiveInit(bytes) {
        ...
        for (const trak of tracks) {
          ...
          const handler = String.fromCharCode(
            bytes[hdlr.dataStart + 8],
            bytes[hdlr.dataStart + 9],
            bytes[hdlr.dataStart + 10],
            bytes[hdlr.dataStart + 11]
          );
        ...
        const trex = mvex && childBoxes(bytes, mvex, "trex").find((box) => box.dataStart + 24 <= box.end && view.getUint32(box.dataStart + 4) === selected.trackId);
        if (trex) {
      ...
      function filterLiveInitToTrack(bytes, trackId) {
        ...
        return concatByteParts(topLevel.map((box) => box === moov ? filteredMoov : copyIsoBox(bytes, box)));
      }
      ...
      function parseTrunDataLayout(bytes, moof, traf, tfhd) {
        ...
        for (const trun of childBoxes(bytes, traf, "trun")) {
          ...
          for (let sampleIndex = 0; sampleIndex < sampleCount; sampleIndex += 1) {
            ...
            if (flags & 1024) offset += 4;
            if (flags & 2048) offset += 4;
      ...
      function filterLiveMediaToTrack(bytes, trackId) {
        ...
        for (let index = 0; index < topLevel.length; index += 1) {
          ...
          const selectedTraf = trafs.find((traf) => {
            const tfhdBox2 = childBox(bytes, traf, "tfhd");
            return tfhdBox2 && tfhdBox2.dataStart + 8 <= tfhdBox2.end && new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(tfhdBox2.dataStart + 4) === trackId;
          });
          ...
          const otherChildren = children.filter((child) => child.type !== "traf").map((child) => copyIsoBox(bytes, child));
      ...
      function isSyncSample(sampleFlags) {
        ...
        if (sampleFlags & 65536) return false;
      ...
      function parseLiveMedia(bytes, init, timestampOffset = 0) {
        ...
        for (const moof of moofs) {
          for (const traf of childBoxes(bytes, moof, "traf")) {
            ...
            let decodeTime = version === 1 ? readUint64(view, tfdtBox.dataStart + 4) : BigInt(view.getUint32(tfdtBox.dataStart + 4));
            ...
            for (const trun of childBoxes(bytes, traf, "trun")) {
              ...
              for (let sampleIndex = 0; sampleIndex < sampleCount; sampleIndex += 1) {
                ...
                let sampleFlags = sampleIndex === 0 && firstSampleFlags !== null ? firstSampleFlags : tfhd.defaultSampleFlags;
                if (flags & 256) {
                  if (offset + 4 > trun.end) break;
                  duration = view.getUint32(offset);
                  offset += 4;
                }
                if (flags & 512) offset += 4;
                if (flags & 1024) {
                  if (offset + 4 > trun.end) break;
      ...
      var LiveRewindTrack = class {
        ...
        ingest(value, { mimeType = this.mimeType, timestampOffset = 0 } = {}) {
          ...
          let pending = this.pendingBytes.byteLength ? concatByteParts([this.pendingBytes, incoming]) : new Uint8Array(incoming);
          let accepted = false;
        ...
        appendSegment(segment) {
          ...
          if (this.timestampOffset !== null && Math.abs(offset - this.timestampOffset) > 1e-4) this.clearMedia();
        ...
        snapshot() {
          ...
          for (let index = 0; index < this.segments.length; index += 1) {
            for (const keyframe of this.segments[index].keyframes) {
              ...
              if (keyframe <= desiredStart + 1e-4) selectedIndex = index;
            }
          }
          ...
          return {
            ...
            liveWallClockStartMs: this.segments[this.segments.length - 1].receivedAtMs - Math.max(0, latestEnd - mediaStart) * 1e3,
      ...
      function installLiveMediaCollector(pageWindow2, enabled) {
        const MediaSourceClass = pageWindow2?.MediaSource;
        const URLClass = pageWindow2?.URL;
        if (!MediaSourceClass?.prototype?.addSourceBuffer || !URLClass?.createObjectURL) return null;
        const sourceTracks = /* @__PURE__ */ new WeakMap();
        const bufferTracks = /* @__PURE__ */ new WeakMap();
        const bufferSources = /* @__PURE__ */ new WeakMap();
        const urlSources = /* @__PURE__ */ new Map();
        const originalAddSourceBuffer = MediaSourceClass.prototype.addSourceBuffer;
        const SourceBufferClass = pageWindow2.SourceBuffer;
      ...
      function scanDocumentVideo(rootDocument) {
        ...
        const candidates = videos.map((video) => {
          ...
          const visible = rect.width > 120 && rect.height > 80 && style?.display !== "none" && style?.visibility !== "hidden" && Number(style?.opacity || 1) > 0;
          return { video, area: visible ? rect.width * rect.height : 0 };
        }).filter((item) => item.area > 0 && item.video.videoWidth > 0 && item.video.videoHeight > 0).sort((a, b) => b.area - a.area);
        return candidates[0]?.video || videos.find((video) => video.readyState >= 1 && video.videoWidth > 0) || null;
      }
      ...
      function startVideoFramePump(video, fps, onFrame, onError) {
        const frameInterval = 1e3 / Math.max(1, Number(fps) || 1);
        ...
        const stop = () => {
          ...
          try {
            ...
            debugLog("startVideoFramePump.stop", error);
          }
          callbackId = 0;
        };
        const schedule = () => {
          if (stopped) return;
          callbackId = useVideoFrames ? video.requestVideoFrameCallback(tick) : requestAnimationFrame(tick);
        };
        const tick = (now) => {
          if (stopped) return;
      ...
      function createCanvasRecorder(capture, fps) {
        ...
        const options = { videoBitsPerSecond: capture.width >= 640 ? 6e6 : 4e6 };
      ...
      function cleanupRecordingResources(recording) {
        ...
        try {
          ...
          debugLog("cleanupRecordingResources", error);
        }
      }
      function createCanvasRecording(video, selection, {
        ...
        const finish = () => {
          ...
          recording.measuredDuration = Math.max(0, (performance.now() - recording.startedAt) / 1e3);
          recording.sourceEnd = Number(video.currentTime) || recording.snapshot.sourceStart + recording.measuredDuration;
          cleanupRecordingResources(recording);
          try {
            video.playbackRate = recording.snapshot.playbackRate;
          } catch (error) {
            debugLog("createCanvasRecording.finish", error);
          }
          try {
            video.pause();
          } catch (error) {
        ...
        recording.stop = (reason = "manual") => {
          ...
          try {
            ...
            debugLog("createCanvasRecording", error);
          }
        ...
        recording.start = async () => {
          try {
            ...
            recording.maxTimerId = setTimeout(
              ...
              Math.max(1, Number(maxSeconds) || 60) * 1e3
            );
            await video.play();
          } catch (error) {
      ...
      function createFrameCompositor() {
        ...
        const draw = (ctx, settings, source, fallbackWidth = 0, fallbackHeight = 0, binaryAlpha = false) => {
          ...
          const radius = Number(settings.outputRadius) || Math.min(width, height) * bound(Number(settings.cornerRadiusRatio) || 0, 0, 0.5);
        ...
        const startObserving = () => {
          ...
          observer = new MutationObserver((records) => {
            const changed = records.some((record) => {
              ...
              return [...record.addedNodes, ...record.removedNodes].some((node) => node instanceof HTMLVideoElement || node instanceof HTMLSourceElement || node.querySelector?.("video, source"));
            });
            if (!changed) return;
            refreshVideo();
            postReady();
      ...
      function createLiveFrameClient({ onReady, onStatus, onRecordingComplete } = {}) {
        ...
        const findFrameElement = (target, rootDocument = document) => {
          for (const frame of rootDocument.querySelectorAll("iframe")) {
            try {
              ...
              debugLog("findFrameElement", error);
            }
          }
          return null;
        };
        const createVideoProxy = (source, frameId) => ({
          ...
          get videoWidth() {
            return Number(peers.get(source)?.video?.videoWidth) || 0;
          },
          get videoHeight() {
            return Number(peers.get(source)?.video?.videoHeight) || 0;
          }
        });
        const register = (source, data) => {
          ...
          peers.set(source, {
            ...
            proxy: current.proxy && current.frameId === (data.frameId || current.frameId || "") ? current.proxy : createVideoProxy(source, data.frameId || current.frameId || "")
          });
        ...
        const handleMessage = (event) => {
          ...
          const request2 = pending.get(data.requestId);
          if (request2.source !== source) return;
          ...
          if (data.error) request2.reject(new Error(data.error));
          else request2.resolve(data);
        };
        ...
        const pingDocumentFrames = (rootDocument = document) => {
          for (const frame of rootDocument.querySelectorAll("iframe")) {
            try {
              ...
              debugLog("pingDocumentFrames", error);
            }
          }
        };
        window.addEventListener("message", handleMessage);
        ...
        return {
          getVideos() {
            return [...peers.values()].filter((peer) => peer.frame?.isConnected && peer.video?.videoWidth > 0 && peer.video?.videoHeight > 0).map((peer) => peer.proxy);
          },
        ...
        try {
          ...
          return Object.freeze({
            async *framesAt(targetTimes, { signal = null } = {}) {
              ...
              let heldSample = null;
              try {
                for await (const sample of sink.samples(void 0, void 0, { skipLiveWait: true })) {
                  ...
                  try {
                    ...
                    while (index < times.length && sample.timestamp - times[index] > 1e-10) {
                      ...
                      if (signal?.aborted) throw new CancelledError();
                      const frame = heldSample.toVideoFrame();
                    ...
                    try {
                      ...
                      debugLog("createExportFrameSourceFromMediaApi", error);
                    }
                    heldSample = sample;
                    sampleHeld = true;
                  } finally {
                ...
                while (index < times.length) {
                  if (signal?.aborted) throw new CancelledError();
                  const sampleEnd = heldSample ? heldSample.timestamp + Number(heldSample.duration) : Number.NaN;
                  if (!heldSample || times[index] + 1e-10 < heldSample.timestamp || !Number.isFinite(sampleEnd) || times[index] >= sampleEnd - 1e-10) {
                    throw new Error(`无法解码 ${times[index].toFixed(3)} 秒的画面。`);
                  }
                  const frame = heldSample.toVideoFrame();
                  yield Object.freeze({ index, targetTime: times[index], frame });
          ...
          try {
            ...
            debugLog("createExportFrameSourceFromMediaApi", error2);
          }
          throw error;
        }
      }
      var PREFS_KEY = "biliGifMakerPrefsV1";
      var DEBUG = (() => {
        try {
      ...
      function debugLog(scope, error) {
        if (!DEBUG) return;
        try {
          console.warn(`[bella-gif][${scope}]`, error);
        } catch (_) {
        }
      }
      var LEGACY_PREF_KEYS = Object.freeze({
      ...
      var LEGACY_LOCAL_PREF_FIELDS = Object.freeze([
        "launcherPosition",
        "exportPreferences",
        "liveCaptureMode",
        "aspectSquare"
      ]);
      var LEGACY_GM_PREF_FIELDS = Object.freeze(["shortcut", "panelGeometry"]);
      var prefsCache = null;
      function parseLegacyPref(field, raw) {
        ...
        if (field === "shortcut") return typeof raw === "object" ? raw : void 0;
        try {
          const parsed = typeof raw === "string" ? JSON.parse(raw) : raw;
          if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) return parsed;
        } catch (error) {
          debugLog("parseLegacyPref", error);
        }
        return void 0;
      }
      function migrateLegacyPrefs() {
        ...
        for (const field of LEGACY_LOCAL_PREF_FIELDS) {
          ...
          let raw = null;
          try {
            raw = localStorage.getItem(LEGACY_PREF_KEYS[field]);
          } catch (_) {
            continue;
          }
          const value = parseLegacyPref(field, raw);
          if (value !== void 0) patch[field] = value;
        }
        for (const field of LEGACY_GM_PREF_FIELDS) {
          if (field in prefs) continue;
          let raw = null;
          try {
            raw = GM_getValue(LEGACY_PREF_KEYS[field], null);
          } catch (_) {
            continue;
          }
          const value = parseLegacyPref(field, raw);
      ...
      function writePrefs(patch) {
        ...
        prefsCache = prefs;
        try {
          GM_setValue(PREFS_KEY, prefs);
        } catch (error) {
          debugLog("writePrefs", error);
        }
        return prefs;
      }
      var IS_LIVE_PAGE = location.hostname === "live.bilibili.com";
      var IS_TOP_WINDOW = window.top === window;
      var pageWindow = typeof unsafeWindow !== "undefined" ? unsafeWindow : window;
      var initialLiveCaptureMode = "rewind";
      if (IS_LIVE_PAGE) {
        try {
          initialLiveCaptureMode = readPrefs().liveCaptureMode === "forward" ? "forward" : "rewind";
        } catch (error) {
          debugLog("liveCaptureModeBootstrap", error);
        }
      }
      var localLiveMediaCollector = IS_LIVE_PAGE ? installLiveMediaCollector(pageWindow, initialLiveCaptureMode === "rewind") : null;
      if (!IS_TOP_WINDOW) {
        if (IS_LIVE_PAGE) installLiveFrameAgent(localLiveMediaCollector);
        return;
      }
      var frameReadyListener = null;
      var frameStatusListener = null;
      var frameRecordingCompleteListener = null;
      var liveFrameClient = IS_LIVE_PAGE ? createLiveFrameClient({
      ...
      var liveMediaCollector = IS_LIVE_PAGE ? {
        getFrameVideos() {
      ...
      function startApp() {
        ...
        const ENCODE_TIMEOUT_MS = 6e5;
        const ENCODER_MAX_IN_FLIGHT_PER_WORKER = 2;
        const EXPORT_UI_UPDATE_INTERVAL_MS = 120;
        ...
        const TIMELINE_THUMBNAIL_REFRESH_DELAY_MS = 1e3;
        ...
        shadow.innerHTML = buildPanelTemplate(SCRIPT_NAME);
        ...
        function formatTime(seconds) {
          ...
          return hours > 0 ? `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${secText}` : `${String(minutes).padStart(2, "0")}:${secText}`;
        }
        ...
        function getVideoViewportRect(video) {
          ...
          while (ownerDocument && ownerDocument !== document) {
            let frame = null;
            try {
              frame = ownerDocument.defaultView?.frameElement || null;
            } catch (error) {
              debugLog("getVideoViewportRect", error);
            }
        ...
        function scanMainVideo() {
          ...
          const collectVideos = (rootDocument) => {
            ...
            for (const frame of rootDocument.querySelectorAll("iframe")) {
              try {
                if (frame.contentDocument) collectVideos(frame.contentDocument);
              } catch (error) {
                debugLog("collectVideos", error);
              }
            }
          };
        ...
        function updateModeUi() {
          ...
          el.selectionRecordBtn.textContent = recording ? state.recording?.stopping ? "正在停止…" : "■ 停止" : "● 录制";
          ...
          const showSelectionTools = Boolean(state.pageSelection) && !state.pageSelectionSession && (state.mode === "capture" || state.mode === "recording");
        ...
        function finishLauncherPointer(event, cancelled = false) {
          ...
          try {
            el.launcher.releasePointerCapture?.(drag.pointerId);
          } catch (error) {
            debugLog("finishLauncherPointer", error);
          }
          if (drag.moved) {
            const rect = el.launcher.getBoundingClientRect();
        ...
        function readEditorVideoLayout() {
          ...
          return Object.values(layout).every(Number.isFinite) && layout.width > 1 && layout.height > 1 ? layout : null;
        }
        ...
        function isVisibleViewportTransition(transform) {
          return Math.abs(transform.translateX) >= 0.5 || Math.abs(transform.translateY) >= 0.5 || Math.abs(transform.scaleX - 1) >= 2e-3 || Math.abs(transform.scaleY - 1) >= 2e-3;
        }
        ...
        function finishPanelHeaderDrag(event) {
          ...
          try {
            el.header.releasePointerCapture?.(drag.pointerId);
          } catch (error) {
            debugLog("finishPanelHeaderDrag", error);
          }
        ...
        function finishPanelResize(event, cancelled = false) {
          ...
          try {
            ...
            debugLog("finishPanelResize", error);
          }
        ...
        function calculateTimelineThumbnailAnalysis(images) {
          const colorBuckets = /* @__PURE__ */ new Set();
        ...
        function cleanupClipAttachment(clip, video) {
          ...
          if (attachment) {
            ...
            try {
              ...
              debugLog("cleanupClipAttachment", error);
            }
          }
          try {
            video.pause();
        ...
        function appendMediaSourcePart(sourceBuffer, part) {
          return new Promise((resolve, reject) => {
            ...
            try {
              sourceBuffer.appendBuffer(part);
            } catch (error) {
              finish(error);
            }
          });
        ...
        async function attachClipToVideo(clip, video) {
          ...
          if (clip.kind !== "media-source") {
            video.src = clip.url;
            video.load();
            if (video.readyState < 1) await waitForEvent(video, "loadedmetadata", 1e4);
            return;
          }
          ...
          video.load();
          try {
            await waitForEvent(mediaSource, "sourceopen", 1e4);
            ...
            if (mediaSource.readyState === "open") {
              const bufferedEnd = sourceBuffer.buffered.length ? sourceBuffer.buffered.end(sourceBuffer.buffered.length - 1) : clip.duration;
        ...
        function closeImageBitmaps(frames) {
          for (const frame of frames || []) {
            try {
              ...
              debugLog("closeImageBitmaps", error);
            }
          }
        }
            ...
            for await (const decoded of frameSource.framesAt(targets.map((target) => target.time), { signal })) {
              ...
              try {
                ...
                try {
                  ...
                  debugLog("capturePaletteFrames", error);
                }
              }
            }
            return samples;
        ...
        async function buildTimelineThumbnails(clip, view = null, { captureAnalysis = false } = {}) {
          ...
          try {
            ...
            for (let index = 0; index < canvases.length; index += 1) {
              ...
              const time = range.start + (range.end - range.start) * index / Math.max(1, canvases.length - 1);
              await seekVideo(video, Math.min(time, Math.max(0, clip.duration - 1e-3)), clip.duration, controller.signal);
        ...
        function applyPageMarkerPointerMove(event) {
          ...
          const rect = session.type === "move" ? moveScreenRect(session.startRect, dx, dy, session.mapping.visibleRect) : resizeScreenRect(session.startRect, session.handle, dx, dy, session.mapping.visibleRect);
        ...
        function finishPageMarkerAdjustment(event) {
          ...
          state.pageAdjustSession = null;
          try {
            el.pageSelectionMarker.releasePointerCapture?.(session.pointerId);
          } catch (error) {
            debugLog("finishPageMarkerAdjustment", error);
          }
          updatePageSelectionUi();
        }
        function updateRecordingUi(recording) {
          const timeText = formatRecordTime((performance.now() - recording.startedAt) / 1e3);
        ...
        function beginRecordingSession(recording, { maxTimer = false } = {}) {
          ...
          if (maxTimer) {
            recording.maxTimerId = setTimeout(() => stopRecording("limit"), MAX_RECORD_SECONDS * 1e3);
          }
        }
        ...
        function waitForEvent(target, eventName, timeoutMs = 8e3) {
          return new Promise((resolve, reject) => {
            let settled = false;
            let timer = 0;
        ...
        async function resolveRecordedDuration(video, measuredDuration) {
          ...
          try {
            const changed = waitForEvent(video, "durationchange", 2500).catch(() => null);
            ...
            debugLog("resolveRecordedDuration", error);
          }
          try {
            video.currentTime = 0;
          } catch (error) {
        ...
        async function finalizeRecording(recording) {
          ...
          cleanupRecordingResources(recording);
          try {
            recording.video.playbackRate = recording.snapshot.playbackRate;
          } catch (error) {
            debugLog("finalizeRecording", error);
          }
          try {
          ...
          const measuredDuration = Number.isFinite(recording.measuredDuration) ? Math.max(0, recording.measuredDuration) : Math.max(0, (performance.now() - recording.startedAt) / 1e3);
          const sourceEnd = Number(recording.sourceEnd) || Number(recording.video.currentTime) || recording.snapshot.sourceStart + measuredDuration;
          state.recording = null;
        ...
        function setupEditorForClip() {
          try {
            el.clipVideo.currentTime = 0;
          } catch (error) {
            debugLog("setupEditorForClip", error);
          }
          state.textLayers = [];
          state.activeTextId = null;
        ...
        function applyEditorCropPointerMove(event) {
          ...
          const dx = event.clientX - session.startX;
          const dy = event.clientY - session.startY;
          const rect = session.type === "move" ? moveScreenRect(session.startRect, dx, dy, session.mapping.visibleRect) : state.aspectSquare ? resizeSquareScreenRect(session.startRect, session.handle, dx, dy, session.mapping.renderedRect) : resizeScreenRect(session.startRect, session.handle, dx, dy, session.mapping.renderedRect);
        ...
        function finishEditorCropAdjustment(event) {
          ...
          try {
            el.editorCropBox.releasePointerCapture?.(session.pointerId);
          } catch (error) {
            debugLog("finishEditorCropAdjustment", error);
          }
          animateCropIntoPreview(session.fittedLayout);
        }
        function updateTimelinePlayhead() {
          if (!state.clip) return;
          const duration = Math.max(1e-3, state.clip.duration);
        ...
        function updateTrimUi(seekTarget = null) {
          ...
          if (seekTarget !== null && Number.isFinite(seekTarget)) {
            try {
              el.clipVideo.currentTime = clamp(seekTarget, 0, duration);
            } catch (error) {
              debugLog("updateTrimUi", error);
            }
          }
          updateTimelinePlayhead();
        }
        ...
        function refitTimelineToSelection() {
          ...
          const changed = Math.abs(next.start - state.timelineViewStart) > 5e-4 || Math.abs(next.end - state.timelineViewEnd) > 5e-4;
          if (changed) {
        ...
        function showTimelineHandlePreview(time) {
          ...
          const target = clamp(Number(time) || 0, 0, Math.max(0, state.clip.duration - 1e-3));
          try {
            el.scrubVideo.pause();
          } catch (error) {
            debugLog("showTimelineHandlePreview", error);
          }
        ...
        function queueTimelinePreview(time, type) {
          if (!state.clip) return;
          state.timelinePreviewTarget = clamp(Number(time) || 0, 0, Math.max(0, state.clip.duration - 1e-3));
          ...
          state.timelinePreviewRaf = requestAnimationFrame(() => {
            ...
            seekVideo(el.scrubVideo, target, state.clip.duration, request.signal).then(() => {
        ...
        function handleTimelinePointerDown(event) {
          ...
          try {
            ...
            debugLog("handleTimelinePointerDown", error);
          }
          ...
          const handleRect = handleType === "start" || handleType === "end" ? event.target.getBoundingClientRect() : null;
        ...
        function finishTimelineDrag(event) {
          ...
          try {
            el.timelineTrack.releasePointerCapture?.(drag.pointerId);
          } catch (error) {
            debugLog("finishTimelineDrag", error);
          }
        ...
        function stopTrimPreview({ keepPosition = true } = {}) {
          ...
          if (state.previewSnapshot) {
            try {
              el.clipVideo.playbackRate = state.previewSnapshot.playbackRate;
            } catch (error) {
              debugLog("stopTrimPreview", error);
            }
          ...
          if (!keepPosition && state.clip) {
            try {
              el.clipVideo.currentTime = state.trimStart;
            } catch (error) {
              debugLog("stopTrimPreview", error);
            }
            updateTimelinePlayhead();
        ...
        async function previewTrimmedClip({ startAt = null } = {}) {
          ...
          const check = async () => {
            ...
            updateTimelinePlayhead();
            if (el.clipVideo.currentTime < state.trimEnd - 1e-3 && !el.clipVideo.ended) return;
            jumping = true;
            try {
          ...
          try {
            ...
            const resumeAt = Number.isFinite(startAt) ? clamp(startAt, state.trimStart, Math.max(state.trimStart, state.trimEnd - 1e-3)) : Number.isFinite(current) && current >= state.trimStart && current < state.trimEnd - 1e-3 ? current : state.trimStart;
            el.clipVideo.pause();
            ...
            state.trimPreviewCleanup = () => {
              try {
                ...
                debugLog("previewTrimmedClip", error);
              }
              stop();
            };
        ...
        function finishTextLayerDrag(event) {
          ...
          try {
            ...
            debugLog("finishTextLayerDrag", error);
          }
          scheduleEditorPreviewRender();
        }
          ...
          return new Promise((resolve) => {
            ...
            if (typeof video.requestVideoFrameCallback === "function") {
              try {
                ...
                debugLog("waitForFreshFrame", error);
              }
            }
        ...
        function seekVideo(video, targetTime, knownDuration = null, signal = null) {
          return new Promise((resolve, reject) => {
            ...
            const duration = Number.isFinite(knownDuration) ? knownDuration : Number.isFinite(video.duration) ? video.duration : targetTime;
            const target = clamp(targetTime, 0, Math.max(0, duration - 1e-3));
            if (Math.abs(video.currentTime - target) < 8e-3 && video.readyState >= 2) {
        ...
        function updatePreviewCanvasLayout(settings = null) {
          ...
          const nextSettings = settings || (() => {
            try {
              return readOutputFrameSettings();
            } catch (_) {
              return null;
            }
          })();
    ...
    `;
        ...
        async function createGifEncodingSession(settings, {
          ...
          return {
            async addFrame(frame, index, delay, sourceVideo = null) {
              let posted = false;
              let waitingForCapacity = false;
              try {
                while (!settled) {
                  const workerIndex = selectEncoderWorker(
                    workers.map((item) => item.inFlight),
                    ENCODER_MAX_IN_FLIGHT_PER_WORKER
                  );
                  if (workerIndex >= 0) {
                  ...
                  if (!waitingForCapacity) {
                    ...
                    try {
                      ...
                      debugLog("createGifEncodingSession", error);
                    }
                  }
        ...
        function readOutputFrameSettings() {
          ...
          return Object.freeze({
            ...
            textLayers: state.textLayers.filter((layer) => String(layer.text || "").trim()).map((layer) => ({ ...layer }))
          });
        }
        function readExportSettings() {
          ...
          const quality = Object.hasOwn(GIF_QUALITY_PRESETS, el.qualitySelect.value) ? el.qualitySelect.value : "bei";
        ...
        function updateEstimatedFileSize({ actualBytes = 0 } = {}) {
          ...
          const bytesPerPixelFrame = 0.035 + analysis.colorComplexity * 0.08 + analysis.motionComplexity * 0.22;
          ...
          const bytes = 1024 + plan.outputWidth * plan.outputHeight * plan.finalFrames * bytesPerPixelFrame * qualityFactor * overlayFactor * cornerFactor;
        ...
        async function generateGif() {
          ...
          const reportExportProgress = (phase, completed, total) => {
            const now = performance.now();
            if (completed < total && now - lastUiUpdateAt[phase] < EXPORT_UI_UPDATE_INTERVAL_MS) return;
            lastUiUpdateAt[phase] = now;
            setProgress(calculateExportProgress(phase, completed, total));
            setStatus(phase === "encoding" ? `正在并行编码：${completed}/${total}` : `正在提取画面：${completed}/${total}`);
          };
          try {
            ...
            encodingSession = await createGifEncodingSession({
              ...
              onProgress: (phase, completed, total) => {
                ...
                reportExportProgress(phase, completed, total);
              }
            }, {
        ...
        function updateLiveRewindTitle(status = null) {
          ...
          el.liveRewindModeBtn.title = state.liveCaptureMode === "rewind" ? `已缓存 ${seconds.toFixed(1)} 秒` : "切换后开始缓存直播画面";
        }
        ...
        const videoObserver = new MutationObserver((records) => {
          const changed = records.some((record) => {
            ...
            return [...record.addedNodes, ...record.removedNodes].some((node) => node instanceof HTMLVideoElement || node instanceof HTMLIFrameElement || node.querySelector?.("video, iframe"));
          });

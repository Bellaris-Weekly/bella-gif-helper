...
| T9 复查遗留问题 | ✅ 完成 | 1.5.7-beta | 见文末 |
| T8 拆分 startApp | 🔻 降级 | — | 不再作为项目推进，改为按需进行，见文末「T8 的处置」 |

> 实际版本号走的是 `1.5.x-beta` 线，与下方 T1–T7 各节里写的 `1.4.x` 规划不同——
> 以本表为准，各节里的版本号仅作历史参考。

**本轮优化已于 2026-09-04 收尾，发布 `1.5.8`（去掉 beta 后缀，转正式版）。**
9+1 项手动冒烟清单已由维护者在真实浏览器中完整跑通并确认通过，
包含「用装过 1.4.x 的浏览器 profile 升级、确认旧设置完整迁移」这一条。
...
1. ~~**发布通道隔离**~~ —— **已决定：保持现状**。
   ...
   `-beta` 后缀只是命名、不影响分发。维护者在知情后选择接受这一现状，不再拆分通道。

   由此产生的硬性要求：**没有灰度缓冲，合并到 main 之前的验证就是唯一防线。**
   `npm run verify` 全绿 + 完整跑一遍手动冒烟清单，两者缺一不可；
   `sync-r2.yml` 里 upload job 的 `needs: test` 不得放宽或绕过。

2. ✅ **手动冒烟清单** —— 1.5.8 发布前已完整跑通。

3. ✅ **CI 重复触发** —— `test` job 加了 `if` 条件：同仓库分支的 PR 由 push 事件覆盖，
   只有 push 和来自 fork 的 PR 才跑，同一个 commit 不再跑两遍。

4. ⏳ **补测试（唯一遗留项）** —— T2–T5 改了运行时行为，但 47 个用例仍然只覆盖
   重构前就存在的纯函数层，新代码零覆盖。风险最高的是 T2 的存量迁移：
   老用户设置丢失是**静默失败**，用户不会报、维护者也不会发现。

   `parseLegacyPref()` / `migrateLegacyPrefs()` 是纯函数，是性价比最高的切入点。
   在补上之前，每次动偏好相关代码都必须手工验证一次「1.4.x profile 升级」这条路径。

---

## T8 的处置：降级为按需进行

**T8（把 4254 行的 `startApp` 拆成 6 个模块）不再作为项目推进。** 理由：

1. **代码本身没有问题。** 复查实测：0 个死函数、0 个死常量、1 处重复块、
   资源释放（`frame.close()` / `worker.terminate()` / `revokeObjectURL`）全部成对。
   巨型函数写在纸上难看，但它当前**没有在制造 bug**。
2. **收益对用户为零。** 拆完不会更快、不会更稳、不会多功能。唯一实质收益是「可测性」。
3. **成本结构因发布通道的决定而改变。** 6 个子任务 = 6 次正式上线，
   每次都要人肉跑完整冒烟清单，且没有灰度缓冲。
4. **存在逻辑死结。** T8 的价值是让代码可测，但 T8 本身就是在没有测试的情况下
   改动这些代码——风险在前、收益在后，而且收益只有在真的补上测试之后才兑现。
   只搬不写测试 = 承担全部风险、收获为零。

### 替代做法

**只保留 T8.1**（把已被 47 个测试覆盖的纯函数层搬出去，顺带删掉
`tests/load-userscript-api.js` 的 `vm` hack，换成正常 `import`）。
这一步有测试兜底，风险接近零，价值是**让以后写任何测试的成本降下来**。

其余部分把顺序反过来：

> 不是「先拆完，将来也许写测试」，而是「想测 X → 就把 X 抽出来」。

每次抽取由一个正要写的测试驱动，当场兑现收益，风险只限于那一小块。
